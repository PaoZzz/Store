...
public abstract class AbstractDAOImpl {
    ...
    /**
     * 实现数据的批量删除，这个时候批量删除属于彻底删除的功能
     * @param table 表名称
     * @param colume 删除表的列名称
     * @param ids   所有的id数据，使用Set可以避免重复
     * @return  如果删除成功返回true，否则返回false
     * @throws Exception
     */
    public boolean removeHandle(String table,String colume,Set<?> ids) throws Exception {
        if (ids.size() == 0) {
            return false;
        }
        StringBuffer buf = new StringBuffer();
        buf.append("DELETE FROM ").append(table).append(" WHERE ").append(colume).append(" IN (");
